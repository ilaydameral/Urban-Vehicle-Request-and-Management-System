...
async function listMyTrips(req, res) {
  try {
    ...
    const driverProfile = await Driver.findOne({ user: req.user.userId }).select("_id");
    if (!driverProfile) {
      return res.status(400).json({ message: "Driver profile not found" });
    }
    ...
    console.error("Driver list trips error:", err);
    return res
      .status(500)
      .json({ message: "Server error while fetching driver trips" });
  }
}
